import { ViewPlugin, Decoration, EditorView, ViewUpdate, type PluginValue } from '@codemirror/view'
import { ReminderWidget } from './widgets'
import { getTaskPrefixRegex, getFullTaskLineRegex, normalizeDateString } from '../taskUtils'
  ...
  class {
    ...
    buildDeco(view: EditorView) {
      ...
      for (const { from, to } of view.visibleRanges) {
        ...
        const reRem = getTaskPrefixRegex()
        while ((match = reRem.exec(text)) !== null) {
          ...
          let isOverdue = false
          const fullRe = getFullTaskLineRegex(false)
          const fullMatch = fullRe.exec(line.text)
          if (fullMatch && fullMatch[4]) {
            const normalized = normalizeDateString(fullMatch[4])
            if (normalized && new Date(normalized).getTime() < Date.now()) isOverdue = true
          }
...
class RemConverterPlugin implements PluginValue {
  ...
  update(update: ViewUpdate) {
    ...
    while ((match = re.exec(docStr)) !== null) {
      ...
      const timestamp = `(${dd}-${mm}-${yyyy} ${hh}:${mins})`
...
export const remConverterPlugin = ViewPlugin.fromClass(RemConverterPlugin)
